...
class TagService:
    ...
    @staticmethod
    async def find_existing_tag_by_names(db: AsyncSession, tag_names: Dict[str, str]) -> Tag | None:
        if not tag_names:
            return None
        ...
        for tag in existing_tags:
            ...
            requested_lang_names = {(lang_code, tag_name) for lang_code, tag_name in tag_names.items()}
    ...
    @staticmethod
    async def add_missing_tag_names(db: AsyncSession, tag: Tag, tag_names: Dict[str, str]) -> None:
        existing_lang_codes = {tn.lang_code for tn in tag.names}

        for lang_code, tag_name in tag_names.items():
            if lang_code not in existing_lang_codes:
                new_tag_name = TagName(tag_id=tag.id, name=tag_name, lang_code=lang_code)
                db.add(new_tag_name)
    ...
    @staticmethod
    async def link_company_to_tag(db: AsyncSession, company_id: int, tag_id: int) -> bool:
    ...
    async def process_company_tags(
        ...
        for tag_request in tag_requests:
            ...
            existing_tag = await TagService.find_existing_tag_by_names(db, tag_request.tag_name.root)

            if existing_tag:
                await TagService.add_missing_tag_names(db, existing_tag, tag_request.tag_name.root)
                tag_to_link = existing_tag
            else:
                tag_to_link = await TagService.create_tag_with_names(db, tag_request.tag_name.root)
    ...
    @staticmethod
    async def get_tags_in_order(db: AsyncSession, tag_ids: list[int], language: str) -> list[str]:
        if not tag_ids:
            return []

        tag_names_query = await db.execute(
            select(TagName.tag_id, TagName.name).where(TagName.tag_id.in_(tag_ids), TagName.lang_code == language)
        )
        tag_names_result = tag_names_query.all()

        tag_id_to_name = {tag_id: name for tag_id, name in tag_names_result}

        return [tag_id_to_name.get(tag_id, "") for tag_id in tag_ids if tag_id in tag_id_to_name]
    ...
    async def add_tags_to_existing_company(
        ...
        for tag_request in tag_requests:
            ...
            if existing_tag:
                ...
                new_tag = await TagService.create_tag_with_names(db, tag_request.tag_name.root)
